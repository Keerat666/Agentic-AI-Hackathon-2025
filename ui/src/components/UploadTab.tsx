...
export default function UploadTab({ user }: UserProps) {
  ...
  const capturePhoto = async () => {
    ...
    try {
      ...
      formData.append('file', blob, `sample${Date.now()}.jpg`);
      formData.append('transaction_time', Date.now().toString());
      formData.append('user', user?.name || 'NA');

      const callbackUrl = await fetch(
        'https://my-wallet-demo.onrender.com/create',
        {
          method: 'POST',
          headers: {
      ...
      const obj = await callbackUrl.json();
      setWalletUrl(obj.saveUrl);
      console.log(obj);

      const response = await fetch(
        'https://us-central1-graceful-byway-467117-r0.cloudfunctions.net/transaction-process',
        {
          method: 'POST',
          body: formData,
        }
  ...
  <a
    ...
    className="w-full bg-green-600 hover:bg-green-700 text-white h-12 rounded-2xl flex items-center justify-center gap-2 shadow-md text-center text-sm font-medium"
  >
    View / Save Pass to Wallet
  </a>
)}
          ...
          <div className="space-y-4">
            ...
            {recentTransactions.map((transaction) => {
              ...
              return (
                <FinalCard
                  key={transaction.id}
                  className="p-4 border-0 shadow-sm bg-white rounded-2xl"
                >
                  <div className="flex items-center space-x-4">
                    <div
                      className={`w-12 h-12 ${transaction.color} rounded-2xl flex items-center justify-center flex-shrink-0`}
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <span className="font-bold text-lg text-gray-900 ml-auto">
                          {transaction.amount}
                        </span>
                      </div>
